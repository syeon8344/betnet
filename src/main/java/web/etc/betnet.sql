...
CREATE TABLE GamePurchaseList (
    ...
    MemberID INT NOT NULL,                    -- 구매한 회원의 번호 (외래 키)
    PurchaseDate DATE NOT NULL default (current_date),           -- 구매일
    FOREIGN KEY (MemberID) REFERENCES Members(MemberID)  -- 회원 테이블의 외래 키
    ON update cascade
    on delete cascade
);
...
-- TeamBoard 테이블에 한글 샘플 데이터 삽입 (TeamCode가 숫자)
...
-- ChatLogs 테이블에 랜덤 ChatRoomUniqueID와 샘플 데이터 삽입 (50개)
...
-- GamePurchaseDetails 테이블에 샘플 데이터 삽입 (50개)
-- MatchID 형식: 'YYYYMMDD-TEAMID-MATCHID'
